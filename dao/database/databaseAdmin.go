...
func (db *DB) Backup(loc string) {
	...
	//db.Flush()
	logHandler.Database.Printf("[ADM] Backup [...%v.db] data started... %v", db.Name, loc)
	db.Disconnect()
	logHandler.Database.Printf("[ADM] Backup [...%v.db] disconnected", db.Name)
	ioHelpers.Backup(db.Name, loc)
	logHandler.Database.Printf("[ADM] Backup [...%v.db] backup done ends", db.Name)
	db.Reconnect()
	logHandler.Database.Printf("[ADM] Backup [...%v.db] (re)connected", db.Name)
	timer.Stop(1)
	logHandler.Database.Printf("[ADM] Backup [...%v.db] data connection", db.Name)
}
