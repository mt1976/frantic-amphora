...
func (record *TemplateStoreV3) insertOrUpdate(ctx context.Context, note string, auditAction audit.Action, operation op, isPostProcessingRun bool) (*TemplateStoreV3, error) {
	...
	if !isCreateOperation && !(operation == UPDATE && isPostProcessingRun) {
		...
		logHandler.Lock.Printf("[%v,%v] Deferring %v lock for new record until after creation", tableName, record.Raw, operation)
	}
	...
	if isCreateOperation {
		if creator != nil {
			...
			if skip {
				logHandler.Trace.Printf("Custom creator skipped for %v record %v", tableName, record.Key)
		...
		if record.Key == "" {
			logHandler.Trace.Printf("No Key found, Generating new UUID for %v record", tableName)
	...
	if locked {
		record.Lock.Unlock()
		logHandler.Lock.Printf("[%v,%v] Unlocked record after post-processing: %v", tableName, record.Raw, operation)
	}
...
func (record *TemplateStoreV3) unlock(isCreate bool) error {
	record.Lock.Unlock()
	if isCreate {
...
func (record *TemplateStoreV3) checkForDuplicate() error {
	...
	if duplicateCheck != nil {
		...
		if found {
			logHandler.Trace.Printf("A duplicate match for %v, %v has been found", tableName, record.Key)
