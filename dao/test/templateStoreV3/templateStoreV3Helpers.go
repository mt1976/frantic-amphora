...
func templateClone(ctx context.Context, source *TemplateStoreV3) (*TemplateStoreV3, error) {
	if cloner != nil {
		logHandler.Database.Printf("[CLONE] Cloning record %v of %v", source.Key, TableName.String())
		nr, err := cloner(ctx, source)
		logHandler.Database.Printf("[CLONE] Cloning complete for record %v of %v", source.Key, TableName.String())
		return nr, err
	}
	return New(), nil
}
...
func (record *TemplateStoreV3) postCreateProcessing(ctx context.Context) error {
	if postCreate != nil {
		...
		if err != nil {
			logHandler.Error.Printf("Error retrieving worker pool from context for post-create processing for %v Record: %v Error: %v", TableName.String(), record.Key, err.Error())
		...
		if workerPool != nil {
			...
			task := workerPool.SubmitErr(func() error {
				...
				if err != nil {
					...
					record.Lock.Unlock()
					logHandler.Lock.Printf("[%v,%v] Unlocked - POSTCREATE with error %v", TableName.String(), record.Raw, err.Error())
					return err
				}
				record.Lock.Unlock()
				logHandler.Lock.Printf("[%v,%v] Unlocked - POSTCREATE", TableName.String(), record.Raw)
...
func (record *TemplateStoreV3) postUpdateProcessing(ctx context.Context) error {
	if postUpdate != nil {
		...
		workerPool, err := contextHandler.GetWorkerPool(ctx)
		if err != nil {
			logHandler.Error.Printf("Error retrieving worker pool from context for post-update processing for %v Record: %v Error: %v", TableName.String(), record.Key, err.Error())
		...
		if workerPool != nil {
			...
			task := workerPool.SubmitErr(func() error {
				...
				if err != nil {
					record.Lock.Unlock()
					logHandler.Lock.Printf("[%v,%v] Unlocked - POSTUPDATE with error %v", TableName.String(), record.Raw, err.Error())
				...
				record.Lock.Unlock()
				logHandler.Lock.Printf("[%v,%v] Unlocked - POSTUPDATE", TableName.String(), record.Raw)
