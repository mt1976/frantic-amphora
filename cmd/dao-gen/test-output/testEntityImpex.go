...
func (record *TestEntity) ExportRecordToJSON(name string) {
...
func (record *TestEntity) ExportRecordToCSV(name string) error {
	ID := reflect.ValueOf(*record).FieldByName(Fields.ID.String())
	clock := timing.Start(tableName, "Export", fmt.Sprintf("%v", ID))
...
func ImportAllFromCSV() error {
	return importExportHelper.ImportCSV(tableName, &TestEntity{}, templateImportProcessor)
}

// templateImportProcessor is called for each CSV row during import.
func templateImportProcessor(inOriginal **TestEntity) (string, error) {
