...
func GetAll() ([]TestEntity, error) {
	...
	records, err := database.GetAllTyped[TestEntity](activeDBConnection)
	if err != nil {
		clock.Stop(0)
...
func GetAllWhere(field entities.Field, value any) ([]TestEntity, error) {
	...
	records, err := database.GetAllWhereTyped[TestEntity](activeDBConnection, field, value)
	if err != nil {
		clock.Stop(0)
		return nil, err
	}
...
// New returns an empty TestEntity record.
func New() TestEntity {
	return TestEntity{}
}

// Create constructs and inserts a new TestEntity record.
...
func Drop() error {
	...
	return activeDBConnection.Drop(TestEntity{})
}
